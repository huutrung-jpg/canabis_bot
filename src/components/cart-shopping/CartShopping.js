...
import { CustomerInfoPopup } from "./CustomerInfoPopup";  // Import CustomerInfoPopup

export function CartShopping({ cart }) {
    const dispatch = useDispatch();
    const [showCustomerInfo, setShowCustomerInfo] = useState(false);  // State để hiển thị popup thông tin khách hàng
    ...
    const handleBuyNow = () => {
        setShowCustomerInfo(true);  // Hiển thị popup thông tin khách hàng khi nhấn Buy Now
    };

    const handleCloseCustomerInfo = () => {
        setShowCustomerInfo(false);  // Đóng popup thông tin khách hàng
        dispatch(toggleBtnCart(false));  // Đóng luôn giỏ hàng
    };

    return (
        <>
            {showCustomerInfo && <CustomerInfoPopup onClose={handleCloseCustomerInfo} />}  {/* Hiển thị CustomerInfoPopup */}
            {!showCustomerInfo && (
                                    ...
                                    <div className='flex justify-center pr-0 py-3 sm:py-4 sm:pr-6 sm:justify-end'>
                                        <div className='bg-gray-200 w-fit py-3 px-2 rounded-xl border border-gray-500 flex flex-col justify-between items-center sm:flex-row sm:py-6 sm:px-4'>
                                            <div className='mr-0 sm:mr-4'>
                                                ...
                                                <span className='text-base sm:text-xl'>{totalPrice()} ISK</span>
                                            </div>
                                            <button onClick={handleBuyNow} className='text-sm sm:text-lg uppercase bg-black text-white py-1 px-4 rounded hover:bg-gray-500 transition mt-1 sm:mt-0'>buy now</button>
                                        </div>
                                    </div>
                                </>
                        }
